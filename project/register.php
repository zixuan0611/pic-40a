	...
	*/
	...
	function validate_register($address, &$register_error){
		...
		while(!feof($our_file)){
			...
			if ($fields[0] === $address && $address !== ""){ //if address already registered
				$register_error = true;
	...
	*/
	...
	function validate_login_email($address, &$login_email_error){
		...
		while(!feof($our_file)){
			...
			if ($fields[0] === $address && $address !== ""){ //if we found our email address
...
<body id="register_body">
	...
	<form method = "post" action ="<?php echo $_SERVER['PHP_SELF']; ?>">
		<fieldset>
			...
			<input id="name" class="styled_input" type="text" name="name"/></br>
			<label for="pass">password: </label> 
			<input id="pass" class="styled_input" type="password" name="pass"/></br>
			<input type="submit" name="Register" value="Register" class="link_button"/>
		</fieldset>
